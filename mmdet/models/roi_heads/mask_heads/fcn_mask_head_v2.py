...
class FCNMaskHeadV2(BaseModule):
    ...
    def get_label_matrix(self, sampling_result):
        '''
        获取 label matrix
        '''
        pos_assigned_gt_inds = sampling_result.pos_assigned_gt_inds
        # 获取总的sample 的数量
        num_sampling = (sampling_result.neg_bboxes.shape[0] + sampling_result.pos_bboxes.shape[0])
        # 生成 label_matrix
        label_matrix = torch.triu(torch.zeros(num_sampling, num_sampling), diagonal= 1)
        pairs_gts = {}
        # 统计所有的分配到同一个gt 的关系
        for pos_id, pos_assigned_gt_id in zip(sampling_result.pos_inds, pos_assigned_gt_inds):
            pos_id = pos_id.item()
            pos_assigned_gt_id = pos_assigned_gt_id.item()
            if pos_assigned_gt_id not in pairs_gts:
                pairs_gts[pos_assigned_gt_id] = [pos_id]
            else:
                pairs_gts[pos_assigned_gt_id].append(pos_id)
        
        # 将 分配到同一个 gt 的，设置为1
        for pos_assigned_gt_id in pairs_gts:
            ls = len(pairs_gts[pos_assigned_gt_id])
            for i in range(ls):
                for j in range(i+1, ls, 1):
                    label_matrix[pairs_gts[pos_assigned_gt_id][i]][pairs_gts[pos_assigned_gt_id][i]] = 1
        
        return label_matrix
    
    def get_label_matrixs(self,sampling_results):
        '''
        生成 label_matrixes
        '''
        label_matrixs = []
        for sample_result in sampling_results:
            label_matrix = self.get_label_matrix(sample_result)
            label_matrixs.append(label_matrix[None,:])
        return torch.cat(label_matrixs, dim = 0)
